...
| **Transport for NSW Live Traffic** | CCTV camera catalog + frames, New South Wales | [CC BY 3.0 AU](https://opendata.transport.nsw.gov.au/dataset/live-traffic-cameras), attribution required | "Transport for NSW Live Traffic cameras (CC BY 3.0 AU)" |
| **Linkt (Transurban CityLink)** | CCTV camera frames, Melbourne CityLink; catalog snapshotted into `config/cctv_sources.linkt-melbourne.json` because linkt.com.au serves it behind a browser challenge | Public webcams on [linkt.com.au](https://www.linkt.com.au/using-toll-roads/traffic-and-roadworks/melbourne); no published data licence, so treat as courtesy use and credit the operator | "Linkt (Transurban) public webcams" |
| **QLDTraffic API (TMR)** | CCTV camera catalog + frames, Queensland | [CC BY 4.0 AU](https://qldtraffic.qld.gov.au/more/Developers-and-Data/index.html), attribution required; requests carry the public developer key from the API specification unless `QLDTRAFFIC_API_KEY` is set | "State of Queensland (Department of Transport and Main Roads), QLDTraffic API" |
