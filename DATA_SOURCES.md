...
| **GCBA API Transporte (Cámaras de Tránsito)** | CCTV camera catalog + frames, Buenos Aires (requires free registration — `CCTV_ARGENTINA_CLIENT_ID`/`SECRET`; curated-location file pack used otherwise) | [GCBA API Transporte terms](https://apitransporte.buenosaires.gob.ar) (free-registered developer access) | "GCBA API Transporte — Cámaras de Tránsito" |
