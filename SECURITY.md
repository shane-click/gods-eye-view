...
- **Hosting on a public URL needs the password gate.** Setting `GEV_ACCESS_PASSWORD` installs an HTTP Basic Auth check ahead of every route, page and `/api/*` alike (`src/accessGate.js`); only `/healthz` stays open for platform health checks. `render.yaml` is a single-instance Render blueprint that turns it on together with the `GEV_RATELIMIT_*` throttles. The gate protects the proxies and your server-side keys; the browser-side Google Maps key and Cesium token are still visible to anyone who has the password, so restrict them to the hosted hostname at the provider.
