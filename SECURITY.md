...
- **Hosting on a public URL needs the password gate.** Setting `GEV_ACCESS_PASSWORD` installs a sign-in page at `/auth/login` ahead of every route, page and `/api/*` alike (`scripts/access-gate.mjs`). A correct code sets a signed, HttpOnly, SameSite=Lax session cookie for 30 days; HTTP Basic credentials with the same password are accepted for scripts; only `/healthz` stays open for platform health checks. Failed attempts are throttled per client and globally (`scripts/access-gate-rate-limit.mjs`). Behind a reverse proxy set `GEV_TRUST_PROXY=1` so the throttle keys on the real client address. `render.yaml` is a single-instance Render blueprint that turns it on together with the `GEV_RATELIMIT_*` throttles. The gate protects the proxies and your server-side keys; the browser-side Google Maps key and Cesium token are still visible to anyone who has the password, so restrict them to the hosted hostname at the provider.
